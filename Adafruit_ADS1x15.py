...
class ADS1x15:
  ...
  def readADCSingleEnded(self, channel=0, pga=6144, sps=250):
    ...
    #
    # Found that the following conversion check takes about 0.55 ms, 
    # so wait for the remainder of the expected conversion time (M.E. 2013)
    delay = 1.0/sps-0.00055
    time.sleep(delay)  

    # Make sure the conversion is finished
    # If not, wait a little longer(M.E. 2013)
    conversiondone = 0
    while True:
        conversiondone = self.i2c.readList(self.__ADS1015_REG_POINTER_CONFIG, 2)
        conversiondone = (conversiondone[0] >> 7)
        if conversiondone == 1:
            break
